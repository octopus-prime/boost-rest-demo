...
ResponseBody::value_type http_client::execute(http::verb verb, boost::urls::url_view url) const {
    http::request<http::empty_body> req{verb, url, 11};
    req.set(http::field::host, url.host());
    req.set(http::field::user_agent, BOOST_BEAST_VERSION_STRING);
    if constexpr (std::same_as<ResponseBody, json_body>) {
        req.set(http::field::accept, "application/json");
...
// instantiation of execute templates
// template http::response<http::empty_body> http_client::execute(http::request<http::empty_body>&) const;
// template http::response<http::empty_body> http_client::execute(http::request<json_body>&) const;
// template http::response<json_body> http_client::execute(http::request<http::empty_body>&) const;
// template http::response<json_body> http_client::execute(http::request<json_body>&) const;

// template http::empty_body::value_type http_client::execute<http::empty_body>(http::verb, boost::urls::url_view) const;
// template http::empty_body http_client::execute(http::verb, boost::urls::url_view, json_body) const;
template json_body::value_type http_client::execute<json_body>(http::verb, boost::urls::url_view) const;
// template json_body http_client::execute(http::verb, boost::urls::url_view, json_body) const;
